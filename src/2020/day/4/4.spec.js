...
describe("Day 4: Passport Processing", () => {
    describe("part 1: In your batch file, how many passports are valid?", () => {
        it("should handle input", () => assert.strictEqual(response(isValid1, input), 226));
    });
    describe("part 2: In your batch file, how many passports are valid?", () => {
        it("should handle input", () => assert.strictEqual(response(isValid2, input), 160));
    });
});

const response = (isValid, input) => toPassports(input).reduce((a, passport) => a + (isValid(passport) | 0), 0);
...
function isValid2(passport) {
    ...
    const {hgtValue, hgtUnit} = passport.hgt?.match(/^(?<hgtValue>\d+)(?<hgtUnit>(in|cm))$/)?.groups || {}
